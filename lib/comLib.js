		...
		onComplete: function(response) {
			// Server ist nicht erreichbar
		 	if(response.status != 200) {
				console.log("Fehler bei der Serverabfrage: " + response.status);
				...
				if(response.headers["Content-Type"].indexOf("text/xml") !== -1 || response.headers["Content-Type"].indexOf("application/vnd.ogc.wms_xml") !== -1) {
					...
					//TODO: Prüfen, was in xml steht um Fehler bei xml antwort welche nicht von einem WMS
					//kommt abzudecken.
					//Beispiel: Server antwortet xml, ist aber kein WMS-GetCapabilites
					console.log("ist xml");
					mainjs.panel.hide();
					openLeafletTab(url);
				} else {
					//Antwort ist kein xml, gebe Fehler aus
					console.log("kein xml!");
					mainjs.panel.port.emit("fehler", "Die angegebene URL scheint nicht korrekt zu sein.");
				}
		}
	}
